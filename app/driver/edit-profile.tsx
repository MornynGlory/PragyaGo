...
export default function DriverEditProfileScreen() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (driver) {
        ...
        if (driver.plate_number && driver.photo_url && driver.pragya_color) setIsProfileLocked(true);
      }
  ...
  const pickPhoto = async () => {
    if (photoUrl) {
      Alert.alert('Photo Locked', 'Your profile photo cannot be changed. Visit any PragyaGo office or station near you to update it.');
      return;
    }
  ...
  const uploadPhoto = async (uri: string) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('driver-photos').upload(fileName, arrayBuffer, { contentType: 'image/jpeg', upsert: true });
      if (uploadError) { Alert.alert('Upload Error', uploadError.message); return; }
      const { data: { publicUrl } } = supabase.storage.from('driver-photos').getPublicUrl(fileName);
  ...
  const saveProfile = async () => {
    if (!isProfileLocked) {
      if (!newPlateNumber.trim()) { Alert.alert('Required', 'Please enter your Pragya plate number.'); return; }
      if (!newPragyaColor) { Alert.alert('Required', 'Please select your Pragya color.'); return; }
      if (!photoUrl) { Alert.alert('Required', 'Please upload your profile photo.'); return; }
    }
    setSaving(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      ...
      if (!isProfileLocked) {
        await supabase.from('drivers').update({ plate_number: newPlateNumber.trim().toUpperCase(), pragya_color: newPragyaColor, photo_url: photoUrl }).eq('profile_id', user.id);
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <View style={styles.loadingContainer}><ActivityIndicator size="large" color="#1D9E75" /></View>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView style={styles.container}>
          ...
          <TouchableOpacity onPress={pickPhoto} disabled={uploading}>
            {photoUrl ? (
              <View>
                ...
                {isProfileLocked && (
                  <View style={styles.photoLockBadge}><Feather name="lock" size={12} color="#1D9E75" /></View>
                )}
              </View>
            ) : (
        ...
        {isProfileLocked && (
          <View style={styles.lockedBanner}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
              <Feather name="lock" size={13} color="#854F0B" />
              <Text style={styles.lockedBannerText}>Identity details are locked. Visit any PragyaGo office to make changes.</Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.section}>
          ...
          {isProfileLocked ? (
            ...
            <>
              <TextInput style={styles.input} value={newPlateNumber} onChangeText={setNewPlateNumber} placeholder="e.g. GR-1234-23" autoCapitalize="characters" placeholderTextColor={colors.subtext} />
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6 }}>
                <Feather name="alert-triangle" size={11} color="#FF9500" />
                <Text style={[styles.warningText, { marginTop: 0 }]}>Cannot be changed after saving without visiting an office.</Text>
              </View>
            </>
          )}
          ...
          {isProfileLocked ? (
            ...
            <>
              <View style={styles.colorsGrid}>
                {PRAGYA_COLORS.map((c) => (
                  <TouchableOpacity key={c.value} style={[styles.colorOption, { backgroundColor: c.color }, newPragyaColor === c.value && styles.colorSelected]} onPress={() => setNewPragyaColor(c.value)}>
                    {newPragyaColor === c.value && <Feather name="check" size={18} color="#fff" />}
                  </TouchableOpacity>
                ))}
              </View>
              {newPragyaColor && <Text style={styles.colorLabel}>Selected: {PRAGYA_COLORS.find(c => c.value === newPragyaColor)?.label}</Text>}
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6 }}>
                <Feather name="alert-triangle" size={11} color="#FF9500" />
                <Text style={[styles.warningText, { marginTop: 0 }]}>Cannot be changed after saving without visiting an office.</Text>
              </View>
        ...
        <View style={styles.section}>
          ...
          <TextInput style={styles.input} value={phone} onChangeText={setPhone} placeholder="024XXXXXXX" keyboardType="phone-pad" placeholderTextColor={colors.subtext} />
        </View>

        <TouchableOpacity style={[styles.saveButton, saving && styles.buttonDisabled]} onPress={saveProfile} disabled={saving}>
          {saving ? <ActivityIndicator color="#fff" /> : (
            <Text style={styles.saveButtonText}>{isProfileLocked ? 'Update Phone Number' : 'Save & Lock Profile'}</Text>
          )}
        </TouchableOpacity>

        <View style={styles.officeCard}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            <Feather name="map-pin" size={14} color="#085041" />
            <Text style={[styles.officeTitle, { marginBottom: 0 }]}>Need to make changes?</Text>
          </View>
          <Text style={styles.officeText}>Visit any PragyaGo office or station near you with a valid ID and your Ghana Card.</Text>
          <View style={styles.officeHoursBox}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <Feather name="clock" size={13} color="#1D9E75" />
...
function makeStyles(c: ReturnType<typeof useTheme>['colors']) {
  return StyleSheet.create({
    ...
    pageHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#1D9E75', paddingHorizontal: 16, paddingVertical: 14 },
    backBtn: { padding: 4 },
    ...
    photoLockBadge: { position: 'absolute', bottom: 0, right: 0, backgroundColor: '#fff', borderRadius: 12, padding: 4 },
    ...
    officeCard: { backgroundColor: '#E1F5EE', margin: 16, marginTop: 0, marginBottom: 32, borderRadius: 12, padding: 16 },
